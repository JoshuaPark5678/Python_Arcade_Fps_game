...
def load_gltf(self, gltf, bin_data, scale=Vec3(0.2, 0.2, 0.2)):
    ...
    if skin:
        ...
        ibm_offset = (ibm_view.byteOffset or 0) + (ibm_accessor.byteOffset or 0)
        ibm_length = ibm_accessor.count * 64
        ibm_data = bin_data[ibm_offset:ibm_offset + ibm_length]
        inverse_bind_matrices = np.frombuffer(ibm_data, dtype=np.float32).reshape((ibm_accessor.count, 4, 4))
    else:
    ...
    for mesh in gltf.meshes:
        for primitive in mesh.primitives:
            ...
            pos_data = np.frombuffer(bin_data[pos_offset:pos_offset + pos_length], dtype=np.float32)
            ...
            if hasattr(primitive.attributes, "TEXCOORD_0"):
                ...
                uv_data = np.frombuffer(bin_data[uv_offset:uv_offset + uv_length], dtype=np.float32).reshape((-1, 2))

            # === JOINTS_0 & WEIGHTS_0 ===
            joints_data = np.zeros((pos_accessor.count, 4), dtype=np.uint8)  # Use uint8 for joints
            ...
            if skin and hasattr(primitive.attributes, "JOINTS_0") and hasattr(primitive.attributes, "WEIGHTS_0"):
                ...
                joint_offset = (joint_view.byteOffset or 0) + (joint_accessor.byteOffset or 0)
                ...
                if joint_accessor.componentType == 5121:  # UNSIGNED_BYTE
                    ...
                    raise ValueError(f"Unsupported JOINTS_0 componentType {joint_accessor.componentType}")

                raw_joints = np.frombuffer(
                    bin_data[joint_offset:joint_offset + joint_accessor.count * 4 * np.dtype(joint_dtype).itemsize],
                    dtype=joint_dtype
                ).reshape((-1, 4))
                ...
                weight_offset = (weight_view.byteOffset or 0) + (weight_accessor.byteOffset or 0)
                weights_data = np.frombuffer(
                    bin_data[weight_offset:weight_offset + weight_accessor.count * 16], dtype=np.float32
                ).reshape((-1, 4))

            # Combine all vertex attributes
            vertex_buffer = self.ctx.buffer(data=np.hstack([pos_data, uv_data]).astype(np.float32).tobytes())
            ...
            if primitive.indices is not None:
                ...
                component_size = {5121: 1, 5123: 2, 5125: 4}[idx_accessor.componentType]
            ...
            geometry = self.ctx.geometry(
                content=[
                    BufferDescription(vertex_buffer, "3f 2f", ["in_pos", "in_uv"]),
                    BufferDescription(joint_buffer, "4u1", ["in_joints"]),  # <-- Use 4u1 for uint8
